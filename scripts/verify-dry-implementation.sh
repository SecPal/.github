...
if [ ! -d "$REPO_ROOT" ]; then
    echo -e "${RED}  ❌${NC} Repository root directory does not exist: $REPO_ROOT"
    exit 1
fi

# Navigate to repository root directory
# Note: All subsequent tests (Test 6-8) assume working directory is the repository root
if ! cd "$REPO_ROOT"; then
    echo -e "${RED}  ❌${NC} Failed to navigate to repository root directory"
    exit 1
fi
