...
11. **Code Coverage Enforcement:** Code coverage tracked via Codecov (see `.codecov.yml`). Minimum 80% coverage for new code, 100% for critical paths. Coverage reports auto-generated in CI and uploaded to Codecov dashboard. PRs must not decrease overall coverage below threshold.
