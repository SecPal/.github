...
HTTP redirects to HTTPS; publication is atomic, is validated with `haproxy -c`,
gracefully reloads HAProxy, and retains last-known-good material.
...
per-deployment Origin authentication before HAProxy admits the request.
...
them; it does not preserve or append caller values.
...
expected Viewer Host, and a natively parsed IPv4 or IPv6 Viewer IP. After
validation, it removes the Origin token and internal Viewer-IP header and
creates canonical XFF anew.
...
independent enforcement layers allow TCP 443 only from the validated AWS
`CLOUDFRONT_ORIGIN_FACING` prefix contract:
...
direct network identity. PROTECTED terminates Viewer TLS and shared WAF at
CloudFront while retaining authenticated HAProxy as the Origin/backend trust
boundary and preserving the real CloudFront TCP peer separately from canonical
Viewer identity.
...
deployment scripts, production credentials, mutable infrastructure state, a
production retention period, or private Fleet orchestration. It does not choose
Falco or Tetragon and does not create downstream implementation contracts before
