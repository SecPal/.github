...
environment and no caller `NODE_PATH`, then proves the tracked source state is
unchanged. Missing or substituted inputs, install failure, or tracked-source
mutation blocks before validation. The generated ignored `node_modules` is
disposable and is not validation evidence. Preparation neither consumes nor
