...
class CollisionCompositionFixture:
    def __init__(
        ...
        *,
        ...
        authenticated_registry: bool = False,
...
'''
    ...
    def collision_validation_reader(self, **arguments):
        source = fast_path.authenticate_integration_commit(
            ...
            head_sha=arguments["resulting_head"],
            expected_signer=arguments["expected_signer"],
            signature_policy={
                "require_github_verified": False,
                "require_local_verified": True,
                "accepted_formats": sorted(self.policy.accepted_formats),
            },
        )
        return (
            self.collision_reader(**arguments),
            self.registry,
            source,
...
def ordinary_ready_provider_growth() -> tuple[
    fast_path.StableFeedbackState,
    fast_path.StableFeedbackState,
    dict[str, object],
]:
    ...
    reviewed, observed, evidence = authenticated_pr_905_classified_codex_review()
    provider_growth = evidence["predecessor_provider_feedback"]
        ...
        "eligible_threads": [
            {
                ...
                "classification": "VALID_ACTIONABLE",
                "disposition": "CORRECTED_AND_VERIFIED",
                "finding_ids": [comments[thread_id]],
...
class LifecycleOrchestrationTests(TestCase):
    def test_post_capture_provider_growth_authorizes_remaining_ordinary_remediation(
        ...
        self.assertEqual(verified.predecessor_state_digest, reviewed.state_digest)
    ...
    def test_post_capture_provider_growth_fails_closed_for_source_and_subset_drift(
        ...
        def reject(label, mutate, *, refresh=True):
            ...
            if refresh:
                reviewed.refresh_digests()
                current.refresh_digests()
                if label != "edited finding body":
                    eligibility["reviewed_head_sha"] = current.head_sha
        ...
        cases = (
            (
                "wrong provider head",
                lambda _r, _c, _e, _h: "7" * 40,
            ),
            (
                "wrong provider actor",
                lambda _r, current, _e, _h: added_review(current).update(
                    actor={"login": "caller", "node_id": "USER_1", "database_id": 1}
                ),
            ),
            (
            ...
            (
                "edited finding body",
                lambda _r, current, _e, _h: added_thread(current)["comments"][0].update(
                    body_digest="7" * 64
                ),
            ),
            (
                "caller finding subset",
                lambda _r, _c, eligibility, _h: eligibility["eligible_threads"].pop(),
            ),
            (
                "fake finding identity",
                lambda _r, _c, eligibility, _h: eligibility["eligible_threads"][0].update(
                    finding_ids=["CALLER_FINDING"]
                ),
            ),
            (
                "resolved late thread",
                lambda _r, current, _e, _h: added_thread(current).update(
                    is_resolved=True
                ),
            ),
            (
                "caller-authored reply",
                lambda _r, current, _e, _h: added_thread(current)["comments"].append(
                    {
                        "node_id": "PRRC_CALLER_REPLY",
                        "body_digest": "8" * 64,
                        "actor": {"login": "caller", "node_id": "USER_1", "database_id": 1},
                        "reply_to_id": added_thread(current)["comments"][0]["node_id"],
                        "reactions": [],
                    }
                ),
            ),
            (
                ...
                lambda reviewed, _c, _e, _h: reviewed.feedback[
                    "conversation_comments"
                ].clear(),
            ),
            (
                "cross-PR replay",
                lambda _r, current, eligibility, _h: (
                    setattr(current, "pull_request_number", 906),
                    eligibility.update(pull_request_number=906),
                ),
            ),
        )
    ...
    def test_provider_growth_authority_composes_only_with_remaining_ready_slot(
        ...
        lifecycle = replace(
            current_lifecycle(
                ...
                head_sha=resulting.head_sha,
                pull_request=resulting.pull_request_number,
                delivery_issue=894,
            ),
            tree_sha="1" * 40,
        ...
        current = publication.VerifiedLifecyclePublication(
            publication_oid="4" * 40,
            publication_digest="5" * 64,
            publication_branch="refs/heads/secpal-lifecycle-publications",
            journal_predecessor_oid="6" * 40,
            predecessor_publication_oid="7" * 40,
            lifecycle=lifecycle,
            serialized_lifecycle_evidence=b"{}",
        )
        predecessor_validation = fast_path._unregistered_validation_evidence(
            repository=REPOSITORY,
            delivery_issue_number=894,
            pull_request_number=resulting.pull_request_number,
            head_sha=resulting.head_sha,
            tree_sha="1" * 40,
            validation_receipt_digest="2" * 64,
        ...
        candidate_validation = fast_path._unregistered_validation_evidence(
            repository=REPOSITORY,
            delivery_issue_number=894,
            pull_request_number=resulting.pull_request_number,
            head_sha="9" * 40,
        ...
        provider_binding = publication.VerifiedReadySourceRecoveryProviderBinding(
            repository=REPOSITORY,
            delivery_issue=894,
            pull_request=resulting.pull_request_number,
            lifecycle_id=lifecycle.lifecycle_id,
            current_head_sha=resulting.head_sha,
            provider_head_sha=reviewed.head_sha,
            ...
            remediation_event_digests=("8" * 64,),
            lifecycle_evidence_digest="9" * 64,
            provider_binding_sources=(publication.ORDINARY_REMEDIATION_SUFFIX,),
        )
        with (
            mock.patch.object(
                fast_path,
        ...
        self.assertEqual(scope["finding_ids"], sorted(verified.finding_ids))
        self.assertEqual(scope["predecessor_head_sha"], resulting.head_sha)
        self.assertEqual(scope["resulting_head_sha"], "9" * 40)
        self.assertRegex(scope["finding_authority_digest"], r"^[0-9a-f]{64}$")

        issued = orchestration.issue_ready_remediation_provider_growth_authorization(
            authorization_id="ordinary-ready-provider-growth",
            reason="Remediate the complete authenticated provider-growth delta",
            current=current,
        ...
        self.assertEqual(authorization["scope"], scope)
        ...
        for label, updates in (
            ...
            ("no remaining remediation", {"remediation_cycle_count": 2}),
            ("second review", {"unrestricted_review_count": 2}),
            ("Cycle 3 present", {"cycle_3_absent": False}),
            ("Ready false", {"ready": False, "draft": True}),
            ("Recovery present", {"exceptional_recovery_count": 1}),
        ):
            changed_state = copy.deepcopy(lifecycle.state)
            changed_state.update(updates)
            changed = replace(lifecycle, state=changed_state)
            changed_current = replace(current, lifecycle=changed)
            with (
...
'''
    ...
    def test_collision_continuation_reuses_authenticated_validation_epoch(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            complete_binding = fixture.collision_validation_reader(
                repository=REPOSITORY,
                delivery_issue=ISSUE,
                pull_request=PR,
                predecessor_head=fixture.predecessor,
                resulting_head=fixture.resulting,
                repository_root=fixture.root,
                expected_signer=fixture.document["expected_signer"],
            )
            sealed, _, authenticated_source, receipt_digest = complete_binding
            with mock.patch.object(
                authority,
                "_load_lifecycle_trust_policy",
                return_value=fixture.policy,
    ...
    def test_historical_collision_binding_preserves_issuer_and_isolates_source(
        self,
    ) -> None:
        from scripts.secpal_pr_review import version_collision
        ...
        with tempfile.TemporaryDirectory() as directory:
            fixture = CollisionCompositionFixture(
                Path(directory), validation_harness=True
            )
            later_main = fixture.commit(
                fixture.git("rev-parse", fixture.main + "^{tree}"),
                "later accepted main",
                fixture.main,
            )
            (fixture.root / version_collision.TRUST_REGISTRY_PATH).write_text(
                "{}\n", encoding="utf-8"
            )
            fixture.git(
                "config",
                "gpg.ssh.allowedSignersFile",
                str(fixture.root / "candidate-controlled-signers"),
            )
            with mock.patch.object(
                version_collision,
                "_authenticate_installed_collision_issuer",
                return_value=later_main,
            ), mock.patch.object(
                authority,
                "_load_lifecycle_trust_policy",
                return_value=fixture.policy,
                ...
                version_collision,
                "_require_historical_collision_issuer",
            ) as issuer_gate, mock.patch.object(
                version_collision,
                "_validation_object_requirements",
                return_value=((fixture.base,), (), ()),
            ), mock.patch.object(
                version_collision,
                "_observe_main",
                return_value=later_main,
            ):
                sealed, binding, source, receipt_digest = (
                    version_collision
                    ._historical_collision_validation_binding_for_commit(
                        ...
                        expected_signer=fixture.document["expected_signer"],
                        expected_collision_digest=fixture.document[
                            "collision_digest"
                        ],
                    )
                )

            issuer_gate.assert_called_once_with(fixture.main, later_main)
            self.assertEqual(sealed.to_dict()["protected_main"], fixture.main)
            self.assertEqual(
                binding["validation"], fixture.registry["validation"]
            )
            self.assertEqual(
                binding["collision_validation_authority"]["validation_epoch"],
                fixture.base,
            )
            self.assertEqual(source.head_sha, fixture.resulting)
            self.assertEqual(source.parent_shas, (fixture.predecessor,))
            self.assertEqual(receipt_digest, fixture.receipt["receipt_digest"])

    def test_collision_issuer_selects_authenticated_validation_epoch(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            def fixture_requirements(path, _blob_oid, source):
                return (
                    version_collision._validation_object_prerequisites(
                        path, source,
                    ),
                    (),
                    (),
            ...
            with mock.patch.object(
                version_collision,
                "_validation_object_requirements",
                side_effect=fixture_requirements,
            ):
                fixture = CollisionCompositionFixture(
                    Path(directory),
                    validation_harness=True,
            ...
            with mock.patch.object(
                ...
                version_collision, "_observe_main", return_value=later_main,
            ), mock.patch.object(
                version_collision, "_require_accepted_issuer",
            ), mock.patch.object(
                version_collision, "_require_historical_collision_issuer",
            ), mock.patch.object(
                version_collision,
                "_accepted_collision_validation_epochs",
                return_value=(later_main, fixture.main),
            ), mock.patch.object(
                version_collision,
                "_validation_object_requirements",
                side_effect=fixture_requirements,
                ...
                lifecycle_execution,
                "_read_live_github",
                return_value=lifecycle_execution.LivePullRequest(
                    REPOSITORY, PR, "OPEN", fixture.predecessor, False,
                ),
            ), mock.patch.object(
                lifecycle_execution,
                "_policy_role_signer",
                return_value=(fixture.identity, fixture.sign),
            ):
                issued = orchestration.issue_collision_continuation_authorization(
                    repository=REPOSITORY,
                    delivery_issue=ISSUE,
                    authorization_id=fixture.document["authorization_id"],
                    reason="Exact independent version collision",
                    resulting_head=fixture.resulting,
                    evidence=evidence,
                )
            ...
            with mock.patch.object(
                ...
                verified = orchestration._verify_user_authorization(
                    issued, fixture.observed, fixture.lifecycle,
                )
            self.assertEqual(
                verified["scope"]["collision"]["protected_main"],
                fixture.main,
            )
            self.assertEqual(
                fast_path.digest_json(
                    version_collision.validation_collision_projection(
                        verified["scope"]["collision"]
                    )
                ),
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(authority, "_load_lifecycle_trust_policy", return_value=fixture.policy), mock.patch.object(
                ...
                with mock.patch.object(version_collision, "_git", side_effect=bounded_local), mock.patch.object(
                    ...
                    version_collision,
                    "collision_validation_binding_for_issuance",
                    side_effect=fixture.collision_validation_reader,
                ), mock.patch.object(
                    lifecycle_publication,
                    "verify_current_lifecycle_authority",
                    return_value=fixture.observed,
    ...
    def test_collision_historical_readback_authenticates_zero_thread_authority(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(
                ...
                historical_binding = fixture.collision_validation_reader(
                    repository=REPOSITORY,
                    delivery_issue=ISSUE,
                    pull_request=PR,
                    predecessor_head=fixture.predecessor,
                ...
                with mock.patch.object(
                    ...
                    version_collision,
                    "_historical_collision_validation_binding_for_commit",
                    return_value=historical_binding,
                ) as validation_authority:
                    ...
                    with mock.patch.object(
                        ...
                        orchestration.verify_collision_continuation_authority(
                            fixture.document,
                            orchestration_authorization=request["authorization"],
                            reviewed_state_evidence=fixture.reviewed.to_dict(),
                            eligibility_evidence=fixture.eligibility,
                    ...
                    substituted_attestation = copy.deepcopy(fixture.attestation)
                    ...
                    with mock.patch.object(
                        ...
                        orchestration.LifecycleOrchestrationError,
                        "does not bind collision Continuation",
                    ):
                        orchestration.verify_collision_continuation_authority(
                            fixture.document,
                            orchestration_authorization=request["authorization"],
                            reviewed_state_evidence=fixture.reviewed.to_dict(),
                            eligibility_evidence=fixture.eligibility,
                            validation_attestation=substituted_attestation,
                            repository_root=fixture.root,
                            repository=REPOSITORY,
                            delivery_issue=ISSUE,
                            pull_request=PR,
            ...
            self.assertEqual(validation_authority.call_count, 2)
            validation_authority.assert_called_with(
                repository=REPOSITORY,
                delivery_issue=ISSUE,
                pull_request=PR,
                predecessor_head=fixture.predecessor,
...
'''
    ...
    def test_version_inventory_digest_is_ast_runtime_independent(self) -> None:
        from scripts.secpal_pr_review import version_collision

        source = Path(fast_path.__file__).read_bytes()
        expected = version_collision.inventory_from_source(source)
        runtime_dump = version_collision.ast.dump

        def changed_runtime_dump(node, *args, **kwargs):
            dumped = runtime_dump(node, *args, **kwargs)
            if isinstance(node, version_collision.ast.FunctionDef):
                return f"{dumped[:-1]}, future_runtime_field=[])"
            return dumped

        with mock.patch.object(
            version_collision.ast,
            "dump",
            side_effect=changed_runtime_dump,
        ):
            observed = version_collision.inventory_from_source(source)
