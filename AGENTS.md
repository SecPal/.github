...
- Never reply to AI review comments with GitHub comment tools. Fix the code, push, and resolve threads
...
- Do not add AI self-references, generated-by text, promotional AI wording, or AI attribution to commits,
  ...
  explicitly requires documenting AI tooling behavior. This includes `Co-authored-by:` trailers for any
  AI tool. The `commit-msg` hook in `scripts/strip-ai-trailers.sh`
  enforces this automatically; keep it installed in every managed worktree.
...
- Reject self-referential AI wording, generated-by text, tool promotion, or AI
  attribution in code, docs, commits, PRs, issues, changelogs, release notes,
  and UI copy unless the task is explicitly about AI tooling.
