...
- narrowed the rollout-guide `@main` regression guard in `tests/dependabot-auto-merge.sh` so prose can warn against moving refs without falsely tripping the `uses:` pinning check
