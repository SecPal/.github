...
- documented that the source-code footer label is localized using the canonical local term for "source code" (English `Source Code`, German `Quellcode`, canonical local term in other locales); abbreviated or informal forms such as `Source`, `Quelle`, or `Quelltext` are not allowed. The slogan, the `Powered by` prefix, the brand name, and the `AGPL v3+` label remain English on every surface and locale
