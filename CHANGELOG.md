...
- documented that the source-code footer label is localized (`Source` in English, `Quelltext` in German, the canonical local term in other locales) while the slogan and the `Powered by` prefix remain English on every surface and locale
