...
- documented `Powered by SecPal` as the exact compact platform attribution for GuardGuide footers in `docs/brand/footer-wording.md`, `docs/brand/brand-architecture.md`, and `docs/adr/BRAND-0004-footer-wording.md`, placed as a separate footer segment between the GuardGuide brand-plus-slogan lockup and the license wording so the platform relationship survives compact footer space without forcing the long-form `GuardGuide by SecPal` lockup into every footer cell
- documented the brand-plus-slogan en-dash lockup (`SecPal – A guard's best friend`, `GuardGuide – A guard's source of truth`) in `docs/brand/slogans.md` and `docs/brand/footer-wording.md` with an explicit U+2013 reference so compact public surfaces stop substituting hyphens for the approved en-dash
...
- locked the slogan rules in `docs/brand/slogans.md` and `docs/brand/footer-wording.md` so the approved English slogans and the `Powered by SecPal` attribution are the only approved wording for every locale and every surface, including German-language pages, and translations, paraphrases, abbreviations, and local campaign variants are not allowed
