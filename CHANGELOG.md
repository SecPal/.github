...
- documented the fixed two-line public AGPL footer in `docs/brand/footer-wording.md`, `docs/brand/brand-architecture.md`, and `docs/adr/BRAND-0004-footer-wording.md`: line 1 is `Powered by <Brand> – <Slogan>` linked to the brand's homepage (`SecPal` → `https://secpal.app`, `GuardGuide` → `https://guardguide.de`), and line 2 is the icon-prefixed `AGPL v3+` label (linked to `https://www.gnu.org/licenses/agpl-3.0.html`) plus a localized source-code label (linked to the SecPal GitHub organization or the GuardGuide repository, depending on surface)
