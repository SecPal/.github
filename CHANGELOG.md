...
- documented the canonical two-line public AGPL footer in `docs/brand/footer-wording.md`, `docs/brand/brand-architecture.md`, and `docs/adr/BRAND-0004-footer-wording.md`: line 1 is `Powered by <Product> – <Slogan>` linked to the brand's homepage (`SecPal` → `https://secpal.app`, `GuardGuide` → `https://guardguide.de`), and line 2 is `AGPL v3+ | <Source Code label>` with `AGPL v3+` linked to `https://www.gnu.org/licenses/agpl-3.0.html` and the Source Code label linked to the SecPal GitHub organization or the GuardGuide repository depending on surface
- documented the brand-plus-slogan en-dash separator rule (`–` U+2013 with one space on each side) in `docs/brand/slogans.md` and `docs/brand/footer-wording.md` with explicit incorrect examples for hyphen-minus (`-` U+002D) and em dash (`—` U+2014) so compact public surfaces stop drifting between dash characters
- documented the line-2 license/source-code separator rule (`|` U+007C with one space on each side) in `docs/brand/footer-wording.md` and `docs/adr/BRAND-0004-footer-wording.md` so the two line-2 labels are never joined with `/`, `·`, `•`, `,`, or a space-only gap
...
- documented that the source-code footer label is localized using the canonical local two-word term for "source code" (English `Source Code`, German `Quellcode`, canonical local term in other locales); one-word abbreviations such as `Source`, `Quelle`, or `Quelltext` are not allowed. The slogan, the `Powered by` prefix, the brand name, and the `AGPL v3+` label remain English on every surface and locale
...
- added `.context/` comment annotation to the existing `.gitignore` entry so the Polyscope agent scratch directory exclusion is self-documenting

**Fixed:**

- corrected the GuardGuide slogan in `docs/brand/slogans.md` and the GuardGuide example footer in `docs/brand/footer-wording.md` to the approved exact string `A guard's source of truth`, replacing the earlier placeholder wording so the shared brand docs no longer publish an unapproved slogan
- corrected the GuardGuide footer attribution from the earlier draft pattern (`Powered by SecPal` on GuardGuide surfaces) to each brand owning its own self-attribution (`Powered by GuardGuide` on GuardGuide surfaces, `Powered by SecPal` on SecPal surfaces) so the line-1 link target on each footer matches the brand named in that line
- locked the slogan rules in `docs/brand/slogans.md` and `docs/brand/footer-wording.md` so the approved English slogans and the `Powered by` prefix are the only approved wording for every locale and every surface, including German-language pages, and translations, paraphrases, abbreviations, and local campaign variants are not allowed
- limited the new AGPL footer and licensing wording guidance in `docs/brand/` and `docs/adr/BRAND-0004-footer-wording.md` to AGPL-licensed public surfaces so the shared docs no longer mislabel commercially licensed deployments
- removed the would-be organization-wide runtime UI stack mandate from `docs/design/components.md`, `docs/design/accessibility.md`, `docs/design/forms.md`, and `docs/adr/BRAND-0006-app-ui-stack.md`, keeping framework, component-library, and icon choices in the owning product repositories
