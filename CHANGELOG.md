...
- preserved only template-defined local base values needed for workspace bootstrap when generating a missing worktree `.env`, still leaving source-only secrets out of new worktrees, avoiding reuse of the source `APP_KEY`, and now carrying forward the template-defined PostgreSQL `DB_PASSWORD` so preview database provisioning can authenticate on first run
- rewrote API preview `KEK_PATH` values to a writable per-worktree storage path so first-run seeders can create tenant key material inside each preview workspace instead of failing against the container-only `/home/appuser/.secrets` path from `api/.env.example`
